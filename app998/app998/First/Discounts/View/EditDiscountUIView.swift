...
struct EditDiscountUIView: View {
            ...
            VStack(alignment: .leading) {
                ...
                Button {
                    if !name.isEmpty, !market.isEmpty, !category.isEmpty, !note.isEmpty {
                        viewModel.editDiscount(discount, name: name, market: market, category: category, startDate: startDate, endDate: endDate, note: note)
                        presentationMode.wrappedValue.dismiss()
                    }
                } label: {
                    ZStack {
...
#Preview {
    EditDiscountUIView(viewModel: DiscountsViewModel(), discount: Discount(name: "Discount 2 for the price of 1", market: "Domino's Pizza", category: "Food", startDate: Date(), endDate: Calendar.current.date(byAdding: .hour, value: 0, to: Date()) ?? Date(), note: "Buy two pizzas and get the second one for free asdsadasd sadsad"))
}
